<template>
  <div class="flex mb-4">
    <div class="w-3/4 bg-grid-color-secondary h-12">
      <vx-card class="fullheight boxcenter2"> 
          ...
          <div class="vx-col sm:w-1/2 md:w-1/2 lg:w-3/4 xl:w-3/5 sm:m-0 m-4">
            <div>
              ...
              <vs-row>
                <vs-col>
                  <div>
                    <h1 class="text-primary mb-3 text-center mt-5 mb-5">
                      Regist you devices
                    </h1>
                  </div>
                </vs-col>
                <vs-col>
                  <div class="mb-5">
                    <img
                      src="@/assets/images/pages/registDevice4.png"
                      width="100%"
                    />
                  </div>
                </vs-col>
      ...
      <vx-card class="fullheight">
        <div>
          <vs-row>
            ...
            <vs-col>
              <vs-divider color="primary"><h4 class="text-primary">List all your devices</h4></vs-divider>
            </vs-col>
            <vs-col>
                      <div>
              <vs-table
    ...
    <vs-popup title="Do you want to name your device?" :active.sync="popupsetnamedevice">
      <vs-input class="inputx mb-3 w-full" placeholder="Devicename" v-model="valueDevicename" />
      <vs-col vs-type="flex" vs-justify="center" vs-align="center" vs-w="12">
          <div class="flex mb-4">
            <div class="w-1/2 px-2 bg-grid-color-secondary h-12">
              <vs-button @click.prevent="submitFormRegisterName" color="primary" type="filled">Submit</vs-button>
            </div>
            <div class="w-1/2 px-2 bg-grid-color h-12 ml-5">
              <vs-button
                @click.prevent="submitFormSkip"
                color="#878787"
                type="filled"
                >Skip</vs-button
              >
...
export default {
  ...
  methods: {
    ...
    submitFormRegister() {
      ...
      else if (this.valueDeviceID != "" && this.valueDevicename == ""){
        this.popupsetnamedevice = true;
      } 
      else {
        if (localStorage.getItem("userIDLogin"))
          this.userLogin = localStorage.getItem("userIDLogin");
    ...
    submitFormRegisterName(){
      this.popupsetnamedevice = false
      if (localStorage.getItem("userIDLogin"))
          this.userLogin = localStorage.getItem("userIDLogin");
        console.log(this.userLogin);
    ...
    submitFormSkip(){
      ...
      if (localStorage.getItem("userIDLogin"))
        ...
        var body = {
          uid: this.userLogin,
          device_id: this.valueDeviceID,
          device_name: this.valueDeviceID
        };
        this.$axios
          .post(this.$api_url + "/userRegistDevice", body)
          .then(response => {
